...
$sql = "SELECT token,e_id,leave_type,from_date,to_date FROM leave_management where leave_status='notapproved'";
$result = $conn->query($sql);
$linkaddr='index.html';
if ($result->num_rows > 0) {
    echo "<table width=600><tr ><th style='padding-left: 50px;color:#4285F4;height:60'>ID</th><th style='padding-left: 35px; color:#4285F4;height:60;width:150%'>Leave type</th>
        <th style='padding-left: 100px; color:#4285F4;height:60'>From date</th>
        <th style='padding-left: 100px; color:#4285F4;height:60'>To date</th>
        <th style='padding-left: 100px; color:#4285F4;height:60'>Approve</th>
        <th style='padding-left: 100px; color:#4285F4;height:60'>Deny</th>
    </tr>";
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<tr ><td style='width:33%;padding-left: 50px; height:60'><a href=leavehistory.php?id=".$row["token"].">".$row["e_id"]."</a></td><td style='width:150%;padding-left: 35px; height:60'>".$row["leave_type"]."</td>
            <td style='width:33%;padding-left: 100px; height:60'>".$row["from_date"]."</td>
            <td style='width:33%;padding-left: 100px;height:60'>".$row["to_date"]."</td>
            <td style='width:33%;padding-left: 100px;height:60'>'<a href=acceptleave.php?myNumber=".$row["token"].">accept </a>'</td>
            <td style='width:33%;padding-left: 100px;height:60'>'<a href=rejectleave.php?myNumber=".$row["e_id"].">reject</a>'</td>
        </tr>";
    }
    echo "</table>";
} else {
